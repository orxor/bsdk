...
namespace BinaryStudio.Security.Cryptography.Interchange
    ...
    public class LDIFFileEntry : IFileService
        ...
        public String FullName { get {
            ...
            AddIfNotNull(values,DomainComponent);
            AddIfNotNull(values,Organization);
            AddIfNotNull(values,Country?.ToLower());
            AddIfNotNull(values,FileName);
            return String.Join("\\", values);
            }}

        private static void AddIfNotNull<T>(IList<T> source,T value)
            where T: class
            {
            if (value != null) {
                source.Add(value);
