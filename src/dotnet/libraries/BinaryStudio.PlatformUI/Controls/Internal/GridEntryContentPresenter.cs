...
namespace BinaryStudio.PlatformUI.Controls.Internal
{
    public class GridEntryContentPresenter : Control {
        ...
        private void EndEdit() {
            var box = this.Descendants<TextBox>().FirstOrDefault(i => i.Focusable);
        ...
        protected override void OnMouseDown(MouseButtonEventArgs e) {
            ...
            if (ContentPresenter != null) {
                ContentPresenter.DoAfterLayoutUpdated(()=>{
                    ...
                    var c = this.Descendants<UIElement>().FirstOrDefault(i => i.Focusable);
                    if (c != null) {
                        c.Focus();
                        }
                    });
        ...
        protected override void OnLostKeyboardFocus(KeyboardFocusChangedEventArgs e) {
            if (e.NewFocus != null) {
                if (e.OldFocus != null) {
                    if (((UIElement)e.NewFocus).Ancestors<UIElement>().Any(i => ReferenceEquals(i, e.OldFocus))) {
                        e.Handled = true;
                        return;
                        }
                    var nparents = ((UIElement)e.NewFocus).Ancestors<UIElement>().ToArray();
                    var oparents = ((UIElement)e.OldFocus).Ancestors<UIElement>().ToArray();
                    if (nparents.Intersect(oparents).Any()) {
                        e.Handled = true;
                        return;
                        }
                    oparents = ((UIElement)e.OldFocus).Ancestors<UIElement>().ToArray();
                    if (nparents.Intersect(oparents).Any()) {
                        e.Handled = true;
                        return;
                        }
