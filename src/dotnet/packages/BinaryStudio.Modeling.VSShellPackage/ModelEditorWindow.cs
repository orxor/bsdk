...
namespace BinaryStudio.Modeling.VSShellPackage
    {
    public class ModelEditorWindow : WindowPane,
            IOleCommandTarget,
            IVsPersistDocData,
            IPersistFileFormat,
            IVsToolboxUser
        ...
        Int32 IPersistFileFormat.Load(String filename, UInt32 Mode, Int32 ReadOnly)
            {
            FileName = filename;
            DataContext = (new Module()).LoadModel(filename);
        ...
        private void PrivateInit()
            {
            ElementHostControl = new System.Windows.Controls.ContentControl();
        ...
        private ContentControl ElementHostControl;
